<template>
	<div>
		...
		<!-- 댓글 영역 -->
		<form id="replyForm">
			<input id="studyNo" v-model="studyNo" name="studyNo" type="hidden"/>
		...
		<!-- 댓글 리스트 반복 -->
		<div v-for="(reply, index) in studyReplyList" :key="reply.rplyNo" class="mb-2 pt-2 px-3">
			<!-- 삭제 되지 않은 댓글일때 -->
	...
	<script>
		module.exports = {
			...
			mounted(){
				...
				this.getStudyNo();  // 스터디 게시글 번호 , 댓글리스트
			...
			methods: {
				...
				getStudyNo() {
					...
					const value = params.get('studyNo'); // 게시글 번호
					
					this.studyNo = value;
	
					this.ReplyList(this.studyNo); // 해당 게시글의 댓글 출력
	
				},
				// 댓글 리스트 출력 
				ReplyList (studyNo){
					axios.post('/studyReplyList', {studyNo : studyNo})
						.then(res => {
							
						this.studyReplyList = res.data; // 해당 게시글의 댓글 리스트
				...
				replyAdd(){
					...
					axios.post('/studyReplyAdd', formData , {
							headers: {
					...
					.then(res => {
						...
						if(res.data.result === 'Success'){
							...
							this.ReplyList(this.studyNo); // 댓글리스트 화면에서 재출력(insert한 댓글 나옴)
        	...
        	event.target.src = 'self/profile.JPG';
				...
				replyModForm(index){
					this.studyReplyList[index].modifyRplyCntn = this.studyReplyList[index].rplyCntn; // 기존 댓글 내용
					this.studyReplyList[index].modFormVisible = !this.studyReplyList[index].modFormVisible;
				},
	
				// 댓글 수정 기능이 일어나는 함수
				replyModifyFn(index){

					let rplyCntn = this.studyReplyList[index].modifyRplyCntn; // 댓글 수정 내용 부분
					let rplyNo = this.studyReplyList[index].rplyNo; // 수정할 댓글번호
		
					axios.post('/studyReplyMod', {rplyNo : rplyNo, rplyCntn : rplyCntn})
						.then(res => {

							// 수정 성공시
							if(res.data.result === 'Success'){

								this.ReplyList(this.studyNo); // 댓글리스트 화면에서 재출력
				...
				// 댓글 삭제 기능이 일어나는 함수
				replyDelete(index){
					let rplyNo = this.studyReplyList[index].rplyNo; // 삭제할 댓글번호
					axios.post('/studyReplyDelete', {rplyNo : rplyNo})
						.then(res => {
