 ...
 * .anyRequest().authenticated()) // 나머지 모든 요청은 로그인해야지만 가능 .formLogin(login ->
 * login.defaultSuccessUrl("/home") //.loginPage("/login")
 * //.usernameParameter("id") //.defaultSuccessUrl("/home") // 로그인 성공시
 * .loginPage("/login") .usernameParameter("userid") .permitAll()) //.and()
 * .logout() .logoutUrl("/logout") .logoutSuccessUrl("/home") // 로그아웃 성공시
 * .permitAll(); // .and() //.csrf().disable(); return http.build();
 * 
 * }
 * 
 * 
 ...
 * 
 * // static 아래에 있는것들은 다 넣어주면됨 // 정적 컨텐츠들은 다 넣어주면됨
 * 
