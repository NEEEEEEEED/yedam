...
function end(char) {
  ...
  for (let i = 0; i < categories.length; i++) {
    if (char == categories[i]) {
      document.getElementById("player" + play + categories[i]).style.display =
        "none";
...
//팝업창 띄우기
/*   function endgame() {
    let width = 100; //정가운데 들어오게
    let height = 40; //정가운데 들어오게
    let left = "45%";
    let top = "40%";
    popup = window.open(
      "popUp.html",
...
/*   document.getElementById("player" + play).style.color = "black";
  ...
  for (let i = 0; i < categories.length; i++) {
    document.getElementById(
      "player" + play + categories[i]
    ).style.backgroundColor = "rgba(230, 228, 213)";
    document.getElementById(
      "player" + play + categories[i] + "result"
    ).style.backgroundColor = "rgba(230, 228, 213)";
    document.getElementById(
